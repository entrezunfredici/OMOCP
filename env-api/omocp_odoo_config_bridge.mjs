/**
 * Bridge Node.js → omocp_odoo.cli_config (Python).
 *
 * SDK_PLUGIN_PATH env var must point to the plugin root directory
 * (where omocp_odoo/ package lives). Defaults to /workspace/plugins/sdk-plugin.
 *
 * PYTHON env var overrides the python command (useful for venv path).
 */
...
/**
 * Call an OdooConfigExecutor action.
 * @param {string} action
 * @param {Record<string, unknown>} payload
 * @returns {Promise<unknown>}
 */
export function sdkOdooConfig(action, payload = {}) {
  return new Promise((res, rej) => {
    const python = resolvePython();
    const child = spawn(python, ["-m", "omocp_odoo.cli_config"], {
      cwd: pluginRoot,
